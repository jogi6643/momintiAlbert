...
<body>
    ...
    // Main export function - replaces your current exportToPDF()
function exportToPDF() {
    console.log('🖨️ Starting PDF export for all pages...');
    
    // Show loading indicator
    updateStatus('Preparing PDF export...', 'status-saving');
    
    // Save current content before printing
    if (typeof saveCurrentContent === 'function') {
        saveCurrentContent();
    }
    ...
    console.log(`📄 Found ${pageCount} pages to export`);
    
    if (pageCount === 0) {
        alert('No pages found to export!');
        updateStatus('No pages to export', 'status-error');
        return;
    }
    ...
    setupPrintEventListeners(pageCount);
    
    // Small delay to ensure everything is ready
    setTimeout(() => {
        console.log('🖨️ Opening print dialog...');
        updateStatus(`Opening print dialog for ${pageCount} pages...`, 'status-saving');
  ...
  /* Export code hare 11 June 2025 */
// ===== FORCE ALL 53 PAGES TO PRINT - COMPLETE FIX =====
// This will definitely print all your 53 pages

function exportToPDF() {
    console.log('🖨️ Starting PDF export for ALL 53 pages...');
    
    // Show loading indicator
    updateStatus('Preparing all 53 pages for PDF export...', 'status-saving');
    ...
    // Get ALL document pages - using multiple methods
    let allPages = [];
    
    // Method 1: From documentWrapper
    const documentWrapper = document.getElementById('documentWrapper');
    if (documentWrapper) {
        allPages = Array.from(documentWrapper.querySelectorAll('.document-page'));
    }
    
    // Method 2: Fallback to global search
    if (allPages.length === 0) {
        allPages = Array.from(document.querySelectorAll('.document-page'));
    }
    
    // Method 3: Find by ID pattern if still no pages
    if (allPages.length === 0) {
        for (let i = 1; i <= 100; i++) { // Check up to 100 pages
    ...
    if (pageCount === 0) {
        alert('No pages found to export!');
        updateStatus('No pages found to export', 'status-error');
        return;
    }
    ...
    // CRITICAL: Force prepare ALL pages for printing
    forceAllPagesVisibleAndPrintable(allPages);
    
    // Add AGGRESSIVE print styles
    addAggressivePrintStyles(pageCount);
    ...
    setTimeout(() => {
        console.log(`🖨️ Opening print dialog for ALL ${pageCount} pages...`);
        updateStatus(`Printing ALL ${pageCount} pages...`, 'status-saving');
        
        // Final check before print
        finalPrePrintCheck(allPages);
...
// AGGRESSIVE function to force ALL pages visible and printable
function forceAllPagesVisibleAndPrintable(allPages) {
    console.log('💪 AGGRESSIVELY preparing ALL pages for print...');
    
    // First, ensure document wrapper is visible
    const documentWrapper = document.getElementById('documentWrapper');
    if (documentWrapper) {
        documentWrapper.style.display = 'block';
        documentWrapper.style.visibility = 'visible';
        documentWrapper.style.opacity = '1';
    }
    
    // Force each page to be printable
    allPages.forEach((page, index) => {
        ...
        if (pageContent) {
            ...
            // Remove any empty content flags
            pageContent.removeAttribute('data-empty-for-print');
        }
        ...
        if (pageNumber) {
            ...
            pageNumber.style.setProperty('display', 'block', 'important');
        }
        
        // Add a print-ready class
        page.classList.add('print-ready');
        
        console.log(`✅ Page ${index + 1} forced ready for print`);
    });
    
    console.log(`💪 ALL ${allPages.length} PAGES AGGRESSIVELY PREPARED`);
}

// AGGRESSIVE print styles that FORCE all pages to print
function addAggressivePrintStyles(totalPages) {
    // Remove existing styles
    const existingStyles = document.getElementById('aggressivePrintStyles');
    if (existingStyles) {
        existingStyles.remove();
    }
    
    // Create AGGRESSIVE print styles
    const printStyles = document.createElement('style');
    printStyles.id = 'aggressivePrintStyles';
    
    // Generate specific styles for each page
    ...
    for (let i = 1; i <= totalPages; i++) {
        pageSpecificStyles += `
            #page-${i} {
                ...
                opacity: 1 !important;
                page-break-after: ${i < totalPages ? 'always' : 'avoid'} !important;
                break-after: ${i < totalPages ? 'page' : 'avoid'} !important;
            }
        `;
    }
    ...
    printStyles.textContent = `
        @media print {
            ...
            /* FORCE ALL DOCUMENT PAGES - CRITICAL */
            .document-page {
                display: block !important;
                visibility: visible !important;
                opacity: 1 !important;
            ...
            /* Last page special handling */
            .document-page:last-child {
                page-break-after: avoid !important;
                break-after: avoid !important;
            }
            
            /* FORCE PRINT-READY PAGES */
            .print-ready {
                display: block !important;
                visibility: visible !important;
                opacity: 1 !important;
    ...
    console.log(`💪 AGGRESSIVE print styles added for ${totalPages} pages`);
}

// Final check before printing
function finalPrePrintCheck(allPages) {
    console.log('🔍 FINAL PRE-PRINT CHECK...');
    ...
    allPages.forEach((page, index) => {
        ...
        console.log(`Page ${index + 1} (${page.id}): ${isVisible ? '✅ VISIBLE' : '❌ HIDDEN'}`);
    });
    
    console.log(`🎯 FINAL RESULT: ${visiblePages}/${allPages.length} pages ready for print`);
    
    if (visiblePages < allPages.length) {
        console.warn('⚠️ Some pages may not print!');
    }
}

// Setup print event listeners
function setupPrintEventListeners(pageCount) {
    ...
    function handleBeforePrint() {
        console.log(`🖨️ PRINT DIALOG OPENED FOR ${pageCount} PAGES`);
        updateStatus(`PRINTING ALL ${pageCount} PAGES...`, 'status-saving');
        
        // Last-minute visibility check
        const allPages = document.querySelectorAll('.document-page');
        let printablePages = 0;
        
        allPages.forEach(page => {
            const isVisible = window.getComputedStyle(page).display !== 'none';
            if (isVisible) printablePages++;
        });
        
        console.log(`📊 PRINTABLE PAGES: ${printablePages}/${allPages.length}`);
    }
    
    function handleAfterPrint() {
        console.log('✅ PRINT COMPLETED OR CANCELLED');
        updateStatus(`Print completed for ${pageCount} pages`, 'status-saved');
...
function cleanupAfterPrint() {
    // Remove print styles
    const printStyles = document.getElementById('aggressivePrintStyles');
    if (printStyles) {
        printStyles.remove();
    }
    
    // Remove print-ready classes
    const printReadyPages = document.querySelectorAll('.print-ready');
    printReadyPages.forEach(page => {
        page.classList.remove('print-ready');
    });
    
    console.log('🧹 CLEANUP COMPLETED');
}

// Alternative: Create a print-only version
function createPrintVersion() {
    console.log('📄 Creating dedicated print version...');
    ...
    let printHTML = `
        ...
        <html>
        <head>
            <title>Document - Print Version</title>
            <style>
                body {
                    font-family: 'Times New Roman', serif;
                    font-size: 12pt;
    ...
    allPages.forEach((page, index) => {
        printHTML += `<div class="document-page">${page.innerHTML}</div>`;
    });
...
window.createPrintVersion = createPrintVersion;
window.printDocument = exportToPDF;
...
console.log('💪 AGGRESSIVE PDF Export for ALL 53 pages loaded');
console.log('💡 Use Ctrl+P for regular print, Ctrl+Shift+P for print version');
    </script>
</body>
</html>